...
export type TBook = { author: string; country: string; imageLink: string; language: string; link: string; pages: number; title: string; year: number }
...
const HomePage = () => {
    ...
    const filterByTitle = (value: string) => {
        setTitle(value)
        setBooks(json.filter(book => book.title.includes(title)))
        // Alternatives (Strictement égal)
        // setData(json.filter(book => book.title == title))
        // setData(json.filter(book => book.title.slice(0, 3) == title))
    }

    const filterByCountry = (value: string) => {
        setCountry(value)
        setBooks(json.filter(book => book.country.includes(country)))
    }

    const filterByAuthor = (value: string) => {
        setAuthor(value)
        setBooks(json.filter(book => book.author.includes(author)))
    }

    return (
        <>
            <div className={"flex justify-center ext-right p-1"}>
                <h5>Search By Title: <input type="text" onChange={(e) => filterByTitle(e.target.value)} /></h5>
                <h5>Search By Author: <input type="text" onChange={(e) => filterByAuthor(e.target.value)} /></h5>
                <h5>Search By Country: <input type="text" onChange={(e) => filterByCountry(e.target.value)} /></h5>
            </div>

            <div className={"flex flex-row flex-wrap w-2/3 m-auto justify-start content-center text-center"}>
                {
                    books.length > 0 &&
                    books.map((book, i) => {
